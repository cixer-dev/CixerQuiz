...
from src.model.trivia_data_handlers.concrete_templates.dh_text_texts import \
    DHTextTexts
from src.model.trivia_data_handlers.concrete_templates.dh_text_images import \
    DHTextImages
from src.model.trivia_data_handlers.concrete_templates.dh_text_videos import \
    DHTextVideos
from src.model.trivia_data_handlers.concrete_templates.dh_image_texts import \
    DHImageTexts
from src.model.trivia_data_handlers.concrete_templates.dh_image_images import \
    DHImageImages
from src.model.trivia_data_handlers.concrete_templates.dh_image_videos import \
    DHImageVideos
from src.model.trivia_data_handlers.concrete_templates.dh_video_texts import \
    DHVideoTexts
from src.model.trivia_data_handlers.concrete_templates.dh_video_images import \
    DHVideoImages
from src.model.trivia_data_handlers.concrete_templates.dh_video_videos import \
    DHVideoVideos
...
class TriviaDataHandler:
    """
    Extract, process, and provide relevant data about a generic trivia.
    """

    def __init__(self, trivia_path: str) -> None:
        self.trivia_path = trivia_path
        self.trivia_manifest_filepath = os.path.join(
            self.trivia_path,
            "manifest.json"
        )
    ...
    def _build_level_to_data(self) -> dict[str, Any]:
        ...
        self._append_trivia_duration(sorted_level_to_data)
        return sorted_level_to_data

    def _build_unsorted_level_name_keys_empty(self) -> dict[str, Any]:
        unsorted_level_to_data = {}
        for filename in os.listdir(self.trivia_path):
            filepath = os.path.join(self.trivia_path, filename)
    ...
    def _append_paths(self, sorted_level_to_data: dict[str, Any]) -> None:
        for level_name in sorted_level_to_data:
            filepath = os.path.join(self.trivia_path, level_name)
            sorted_level_to_data[level_name]["level_path"] = filepath

    def _append_trivia_duration(
            self,
            sorted_level_to_data: dict[str, Any]) -> None:
        trivia_duration = reader.read_json_key(
            self.trivia_manifest_filepath,
            "trivia_duration"
        )
        for level_name in sorted_level_to_data:
            sorted_level_to_data[level_name]["trivia_duration"] = \
                trivia_duration
    ...
    def _build_dh_level(self) -> list[LevelInstance]:
        ...
        for level_name, data in self.level_to_data.items():
            ...
            trivia_duration = data.get("trivia_duration")
            level_object = level_class(level_path, trivia_duration)
