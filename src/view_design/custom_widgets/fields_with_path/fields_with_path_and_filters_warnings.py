...
from src.view_design.custom_widgets.file_system_buttons.\
    for_files_with_filters_warning import FilesystemForFilesWithFiltersWarnings


class FieldWithPathAndFilters(FieldsWithPathTemplate):
    """QGridLayout containing a file path default_field with a browse button \
    and filters."""
    ...
    def __init__(
        ...
        self.field_path = FieldForPath(self.field_title)
        self.field_path.sgn_path_changed.connect(self._on_path_changed)
        self._set_default_field(self.field_path)

        self.open_filesystem_btn = FilesystemForFilesWithFiltersWarnings(
            self.parent_widget,
            self.filters_code,
        )
