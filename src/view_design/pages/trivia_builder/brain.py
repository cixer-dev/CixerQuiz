...
from src.model.trivia_builder.brain import TriviaBuilder
...
from src.view_design.pages.trivia_builder.action_bar import BuilderActionBar
from src.view_design.pages.trivia_builder.area.brain import (
    TriviaBuilderArea,
)


class TriviaBuilderMenu(QtW.QWidget):
    """QWidget menu that builds trivia and emits completion/errors."""
    ...
    def __init__(self, parent_widget):
        ...
        self.trivia_builder_area = TriviaBuilderArea(self.parent_widget)
        self.trivia_ops_bar = BuilderActionBar(self.parent_widget)

        self.trivia_builder_area.sgn_manifest_data_changed.connect(
            self._on_trivia_manifest_data_changed
        )
        self.trivia_builder_area.sgn_levels_data_changed.connect(
            self._on_levels_data_changed
        )

        self.trivia_ops_bar.sgn_confirmed_selection.connect(
            self._on_confirmed_selection
        )

        self.container_grid.addWidget(self.trivia_builder_area, 0, 0)
        self.container_grid.addLayout(self.trivia_ops_bar, 1, 0)
    ...
    def _on_trivia_manifest_data_changed(self, new_trivia_manifest_content):
        self.trivia_manifest_content = new_trivia_manifest_content

    def _on_confirmed_selection(self):
        try:
            self.build_trivia_files()
            complete_operation_complete_message_box \
                = CompleteOperationMessageBox(
                    self.parent_widget,
                    "The trivia was made correctly",
                )
    ...
    def _on_accepted_message(self):
        self.parent_widget.switch_to_trivia_menu()

    def build_trivia_files(self):
        trivia_builder = TriviaBuilder(
            self.trivia_manifest_content,
            self.levels_data
        )
        trivia_builder.build_trivia()
