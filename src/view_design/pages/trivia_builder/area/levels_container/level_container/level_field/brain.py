...
from src.view_design.pages.trivia_builder.area.levels_container.\
    level_container.level_field.tb_id_field import TriviaIdField
from src.view_design.pages.trivia_builder.area.levels_container.\
...
class LevelField(QtW.QVBoxLayout):
    """QWidget container for a single trivia level definition."""
    ...
    def __init__(self, parent_widget):
        ...
        self.trivia_id = "TEXT_TEXTS"
        self.num_possible_questions = 1

        self.trivia_id_container = StandardContainer(self)
        self.num_possible_questions_container = StandardContainer(
            self
        )
        self.text_content_container = StandardContainer(self)

        self.trivia_id_field = self._build_id_field()
        self.num_possible_questions_field \
            = self._build_num_possible_questions_field()

        self.question_have_media \
            = self.trivia_id_field.get_questions_have_media()
        self.answers_have_media = self.trivia_id_field.get_answers_have_media()
        self.filters_code = self.trivia_id_field.get_filter_code()
    ...
    def _build_id_field(self):
        trivia_id_field = TriviaIdField(self.parent_widget)
        trivia_id_field.sgn_trivia_id_changed.connect(
            self._on_trivia_id_changed
        )
        self.trivia_id_container.setLayout(trivia_id_field)
        return trivia_id_field

    def _on_trivia_id_changed(self, new_trivia_id):
        self.trivia_id = new_trivia_id
        self.question_have_media \
            = self.trivia_id_field.get_questions_have_media()
        self.answers_have_media = self.trivia_id_field.get_answers_have_media()
        self.filters_code = self.trivia_id_field.get_filter_code()
    ...
    def _build_num_possible_questions_field(self):
        ...
        num_possible_questions_field.sgn_field_content_changed.connect(
            lambda new_num_questions: self.on_trivia_num_questions_changed(
                new_num_questions
            )
        )
    ...
    def on_trivia_num_questions_changed(self, new_num_questions):
