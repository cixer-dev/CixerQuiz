...
from src.model.configurators.trivia_pinner_handler \
    import TriviaPinner


class PinOrUnpinButton(QtW.QPushButton):
    """QPushButton that toggles the pinned trivia status using the \
    TriviaPinner backend."""
    ...
    def __init__(self, trivia_filepath, parent_widget):
        ...
        self.trivia_pinner_handler = TriviaPinner(self.trivia_filepath)
        self.setChecked(self.trivia_pinner_handler.is_pinned)
    ...
    def _on_button_toggled(self, checked):
        self.trivia_pinner_handler.toggle_pin_status()
        self._update_icon_button()
