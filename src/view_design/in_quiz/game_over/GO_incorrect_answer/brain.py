...
from src.view_design.in_quiz.game_over.loss_header import LossHeader
from src.view_design.in_quiz.game_over.GO_incorrect_answer.\
    loss_reason_explanation import LossReasonExplanationPlaintext
from src.view_design.in_quiz.game_over.go_back import GoBack
from src.view_design.in_quiz.game_over.repeat_quiz import RepeatQuiz
from src.view_design.custom_widgets.containers.standard \
    import StandardContainer


class GOIncorrectAnswer(QtW.QWidget):
    """Game-over screen shown when an incorrect answer is selected."""

    def __init__(self, main_stack, data_for_display):
        super().__init__()
        self.main_stack = main_stack
        self.data_for_display = data_for_display
        self.quiz_path = data_for_display["quiz_path"]
        self.correct_answer = data_for_display["correct_answer"]
        self.question = data_for_display["question_text"]
    ...
    def _build_up_grid(self):
        up_grid = QtW.QGridLayout()

        loss_header = LossHeader()
        loss_header.setAlignment(QtC.Qt.AlignmentFlag.AlignCenter)
        loss_reason_explanation = LossReasonExplanationPlaintext(
            self.question, self.correct_answer
        )
    ...
    def _build_down_grid(self):
        ...
        go_back_btn.pressed.connect(self.main_stack.switch_to_quiz_menu)
        repeat_quiz.pressed.connect(
            lambda: self.main_stack.switch_to_in_quiz(self.quiz_path)
        )

        down_grid.addWidget(go_back_btn, 0, 0)
        down_grid.addWidget(repeat_quiz, 2, 0)
